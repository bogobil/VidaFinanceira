  ...
  if (forma === 'cartao') {
    ...
    getDocs(collection(db, 'cartoes')).then(snapshot => {
      snapshot.forEach(doc => {
        const c = doc.data();
        selCartao.append(new Option(`${c.nome} (${c.bandeira_nome || c.bandeira})`, doc.id));
      });
    });
  }
...
function mostrarPrazo() {
  // Prazo já controlado em mostrarPagamentoNo
}
...
async function salvarLancamento() {
  ...
  let nomeUsr = '';
  if (dados.usuario_id) {
    const usSnap = await getDocs(query(collection(db,'usuarios'),where('__name__','==',dados.usuario_id)));
    usSnap.forEach(u => nomeUsr = u.data().nome);
  }
