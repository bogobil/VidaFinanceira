...
function mostrarPrazo() {
  // Condicional já controlada por mostrarPagamentoNo
}
...
function carregarCategorias() {
  ...
  const categorias = {
    ...
    despesa: [
      ...
      'Manutenção Veículo','Plano de Saúde','Medicamentos','Consultas','Exames',
      ...
      'Cosméticos','Eletrônicos','Software','Presentes','Impostos','Seguros','Taxas Bancárias','Diversos'
    ]
  };
  (categorias[tipo] || []).forEach(c => sel.add(new Option(c,c)));
}
...
async function salvarLancamento() {
  ...
  if (dados.usuario_id) {
    ...
    us.forEach(u => usuarioNome = u.data().nome);
  }
  const compraId = `compra_${Date.now()}`;
  const prom = [];
  for (let i=1;i<=dados.prazo;i++){
    const dtd = new Date(dados.data); dtd.setMonth(dtd.getMonth()+i-1);
    const obj = {
      ...dados,
      data: dtd.toISOString().split('T')[0],
